...
enum HeroesGridUI {
  struct RootView: View {
    ...
    private func makeHeroDetails() -> some View {
      LazyView {
        IfLetStoreView(store: store.scoped(to: \.details, event: HeroesGridUI.Event.details)) { store in
  ...
  static var reducer: Reducer<State, Event> {
    Reducer<State, Event>.combine(
      ...
      HeroDetailsUI.reducer
        .optional()
        .pullback(state: \HeroesGridUI.State.details, event: /HeroesGridUI.Event.details)
    )
  }
